...
**① 动效兜底** —— 全仓 47 处动效、**0 处** `prefers-reduced-motion`。在 `globals.css` 加一段媒体查询把动画/过渡压到 `0.01ms`。

⚠️ **为什么敢一刀切压掉**：通常不敢 —— 转圈停下会被读成「卡住了」，那是个假信号。但这套体系有条硬纪律（§1 问题 2）：**图标 + 文字 + 颜色三重线索**。查证过：`animate-spin` 只出现在 `StatusPill` 的 `pending` 等三处，那个图标恒 `aria-hidden="true"`、旁边**永远有文字**；`animate-pulse` 只在骨架屏上，形状本身就是占位信号。⇒ 停掉丢的只是装饰。
⚠️ 用 `0.01ms` 而不是 `0`：有些浏览器对 `0s` 不触发 `animationend`，而 Radix 的退场等这个事件 —— 归零会让元素永远卡在退场中。
⚠️ 钉在 **e2e**（`e2e/reducedMotion.spec.ts`）：这是纯 CSS 媒体查询，typecheck / lint / jsdom **都看不见它**，删掉除了那个文件没有任何一道门会红。另有一条**对照组**（没开这个开关时动效照常）—— 否则把 `@media` 条件删成无条件，正向那条依然全绿，而那会把所有用户的动效都干掉。

**② axe 复验 —— 挖出一处假绿**

`@storybook/addon-a11y` 装了、`main.ts` 挂了、`vitest.storybook.config.ts` 文件头写着「Storybook 交互 / **a11y 测试**」—— **但它一条都不判**。

根因在 addon 自己的 `afterEach`：`getIsVitestStandaloneRun() && hasViolations && getMode()==='failed'`，而 `getIsVitestStandaloneRun = () => import.meta.env.VITEST_STORYBOOK === 'false'`。那个值由 storybook 的 vitest 插件自己 `define`，走插件跑就永远是「非 standalone」⇒ 违规只进报告、不抛断言，**外部环境变量也盖不掉**。

⚠️ 实证时自己骗过自己两次，两次都值得记：
- 第一版探针用**零尺寸空按钮** —— axe 本来就跳过不可见元素。换成有尺寸的才算数。⛔ 探针没触发 ≠ 没有违规。
- 第一版 e2e 把整个 `system` 前缀糊成 `{}`，页面崩了，axe 于是报出 `html-has-lang` / `document-title` 这种**根布局明明有**的项 —— 差点被当成真缺陷去改根布局。⛔ 页面崩了的 axe 结果不是这一页的 a11y 结论。

⇒ 真正的门禁落在 **e2e 的真实渲染整页**（`e2e/a11y.spec.ts`，装了 `axe-core@4.13.0`），与本节原文「用**真实渲染**跑一遍 axe 复验」一致；也比逐个 story 更接近用户看到的东西（组合起来才会出现的重复 id、地标缺失，单个 story 里看不见）。
⚠️ 只收 `violations` 不收 `incomplete`：后者是 axe 自己都拿不准、要人工判的项，当失败会逼着后人去关规则。

**跑出来只有一条真缺陷：四个页面都缺 `<h1>`。** 工作台把已有的可见标题升成 `h1`（零视觉变化）；设置页用 **`sr-only` h1** —— 那一区**刻意不显示页面标题**（「我在哪一页」由左侧菜单的选中态回答），⛔ 不擅自加可见标题改设计，读屏用户拿不到"选中态"这个视觉线索，`h1` 正是给他们的那一份。

**③ 主题切换入口 —— 开放问题就此关闭：给**

三态单选（跟随系统 / 暗色 / 亮色），放在**设置菜单**里而不是顶栏摆个太阳/月亮 —— 它是一次性设好就不再碰的偏好，顶栏那点横向空间留给每天都要点的东西。
⛔ **不做成「暗色」开关**：「跟随系统」不是「暗色」的反面，它是第三种状态；做成开关用户就没法把已经表过的态**收回去**。

⚠️⚠️ **默认是 `dark` 不是 `system`**。我一开始写成 `system`，实测才发现首屏变成亮色 —— 查产品文档 P21「暗色终端风格 | **全局默认暗色**」，那是一次产品裁决，不是没人管的缺省值。把默认改成"跟随系统"等于替产品改主意。`system` 是用户**可以自己选**的第三态，不是出厂值。

⚠️ **首屏闪烁**：偏好存在 localStorage，而 persist 要等 React 水合才读得到 —— 那时第一帧早画完了，亮色用户每次刷新会看到一下刺眼的白→黑→白。⇒ 在 `app/layout.tsx` 的 `<head>` 里放一段**内联同步脚本**，⛔ 不能挪进 `useEffect` / Provider（那些全在水合之后）。整段包 try/catch：localStorage 在隐私模式下会**抛异常**，而这是 head 里的同步脚本，抛出去整页白屏。
⚠️ 这段脚本与 `hooks/_shared/useTheme` 是**同一套判定**，改一处就要改另一处 —— 否则会出现"刷新后是亮的、切一下变暗的"这种自相矛盾。

⚠️ `persist` 白名单守卫红了一条（10 → 11 字段）。**那是它的用途不是麻烦**：逼着加字段的人显式说一句"这个字段为什么可以落盘"，⛔ 而不是让白名单在一次次"顺手加一个"里慢慢长成黑名单。`theme` 与 `sidebarCollapsed` / `terminalFontSize` 同类，纯显示偏好。

⚠️ 亮色走查时我先报了「按钮灰底灰字、提示被遮挡」两个问题，量完是 **16.9 / 19.8** 的对比度、也没重叠 —— 那是 `disabled:opacity-50` 和一个悬浮球造成的错觉。**撤回。**

#### 附带裁决：「未配置」只在卡头说一次（2026-09-16）

凭证页一屏出现 **4 个**橙色「未配置」虚线框，Claude Code 那张卡把同一件事说了三遍（卡头 + 两个子行）。

⚠️ **本原型此前在这点上不自洽**：Codex 的 API Key 行有 pill、Claude Code 的子行没有，而没写下理由 —— 实现看到不一致，就统一加上了。这是原型的锅。

⇒ **子行一律不挂状态标记**，三条理由：

| # | 理由 |
|---|---|
| ① | **按钮文案已经分得开**：未配置是「登录帐号 / 添加 API Key」，已配置是「重新登录 / 更换」。再加标记是把按钮说过的话重说一遍 |
| ② | **卡头已经说过一次** |
| ③ | ⭐ **颜色是误报**：`skipped` 是 warning 橙。而「帐号登录 / API Key **二选一**」是产品明说的 —— 没选的那一路本来就该是空的，**不是缺陷**。给它橙框等于在说"这里有问题" |

⚠️ 橙色要留给**真的用不了**的那一个（整张卡未配置 = 这个 Agent 现在跑不了）。一屏 4 个等重的橙框，反而把那一个淹掉了。改后：橙框 4 → 1，「未配置」出现 4 → 2 次。

⚠️ `skipped` 在**诊断页**是对的（一项检查没跑 = 覆盖率缺口），搬到凭证页就变味了 —— ⛔ 同一个 status 在不同语境下未必通用，加新用法前先问它在那儿说的是不是同一件事。
