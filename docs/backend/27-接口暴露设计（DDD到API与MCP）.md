...
| `syncBaseline` | `POST /api/projects/:id/sync` | — | **`ready` ∧ `git`**（空项目也是 `ready`，只判状态会让 fetch 跑进非仓库目录）；`git fetch --all` + 刷新体积与时间戳；**不动已有 Task 的工作区**；超时 5min | `ProjectDto` | `sync` command | 已有副本不可被改写 | `INVALID_STATE`(409)、`CLONE_FAILED_PERMISSION`(403·不可重试)、`CLONE_FAILED_NETWORK`(502)、`TIMEOUT`(504)、`DISK_INSUFFICIENT`(507) | — |
...
| `provisionPresetImage` | `POST /api/system/preset-image/provision` | — | **SSE `text/event-stream`**：逐阶段 `event: stage`（`plan` → `fetch` → `verify` → `load` → `register`）+ `event: done` | — | **只搬不建**。搬运源三选一：本机 docker 镜像库 / 发布资产清单（`cap-image-assets.json`，按 provider × platform）/ 上游 registry。⚠️ **先验血统再搬**——上游基础镜像搬来照样过不了注册（04 §7 ★），而 `resolve()` 零字节层下载，前置检查很便宜。⚠️ **不做 MCP 面**：这是一次分钟级、写 registry 的部署动作，与 `/init` 同属「不该让上层 agent 替人按下」的那一类（本文 §1 ⛔）|
