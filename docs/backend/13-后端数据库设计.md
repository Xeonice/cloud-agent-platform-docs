...
**共 18 张表**（15 张属上下文 + 3 张平台表）——这是**设计总量**。**真实建出来的是 15 张**（2026-08-31 按同一条 grep 实测）（`grep -h 'CREATE TABLE' api/drizzle/*.sql` 去掉 `__new_*` 临时表后的全量）：`sandboxes`、`sandbox_state_transitions`、`agent_tasks`、`projects`、`credentials`、`credential_sandbox_bindings`、`runtime_settings`、`runtime_installations`、**`images`、`image_manifests`**（后两张随 2026-08 镜像切片建于 `drizzle/0010`）、**`retained_volumes`**（`drizzle/0017`）、**`automations`、`automation_runs`**（`drizzle/0018`，F21-7），以及平台表 `system_settings` 与审计表 `audit_events`。⏳ 仍未建的：`resource_allocations`、`terminal_sessions`、`domain_events`、`nodes`。⚠️ **`audit_events` 不在本文档原来那份 18 张的清单里** —— 它是 §2.8.2 的审计流表，落在 `apps/api` 的平台层；本行按「库里真的有什么」记数，与「设计总量」那一半刻意分开。跨上下文的外键只有 7 条（§2.9 汇总），其余关联一律弱引用——这是 23 §4.2「跨聚合只持 ID」在存储层的落地。
