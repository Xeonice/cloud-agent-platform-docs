...
> - **`version_detected` 与 `isInstalled()` 都必须靠 `command -v` + `--version` 实测，不得由硬编码路径推断**——同一 runtime 在 aio（npm prefix `/opt/nodejs/22`）与 boxlite（`/home/gem/.npm-global`）下装的位置不同（04 §2.1★）；硬编码会让 boxlite 侧永远判"没装"，本表随之被反复写成 `installing`。
