...
**共 18 张表**（15 张属上下文 + 3 张平台表）。跨上下文的外键只有 7 条（§2.9 汇总），其余关联一律弱引用——这是 23 §4.2「跨聚合只持 ID」在存储层的落地。
...
| `agent_endpoint_port` | int | ✅ | NULL | — | **provider 的运行时绑定，平台原样持久化、从不解析**（04 `SandboxHandle`）。`boxlite` 存 in-sandbox API 的宿主 loopback 转发端口——BoxLite `getInfo` 不暴露端口映射，重启后无法再推导，不落库就等于丢掉数据面；`aio` 留 NULL（它每次从 `docker inspect` 重新解析）。⚠️ ADR「影响面」原写"沙箱内 API 端点运行时解析、**不落库**"，只对 `aio` 成立，已按实现更正 |
| `agent_auth_token` | text | ✅ | NULL | — | **每沙箱一枚的 agent bearer token**（RS256 JWT，ADR「安全姿态」加固 1）。**两个 provider 都写**：容器内只有公钥，token 无法从运行时反推，不落库则后端重启后 exec/终端全断。**机密**：不进任何 DTO、不进日志；随 sandbox 行一起消亡。残留风险（同 uid 进程可读 `platform.db`）见 ADR |
