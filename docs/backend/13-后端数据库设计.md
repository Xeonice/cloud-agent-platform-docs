...
| `agent_endpoint_port` | int | ✅ | NULL | — | **provider 的运行时绑定，平台原样持久化、从不解析**（04 `SandboxHandle`）。`boxlite` 存 in-sandbox agent 的宿主 loopback 转发端口——BoxLite `getInfo` 不暴露端口映射，重启后无法再推导，不落库就等于丢掉数据面；`aio` 留 NULL（它每次从 `docker inspect` 重新解析）。⚠️ ADR「影响面」原写"agent 端点运行时解析、**不落库**"，只对 `aio` 成立，已按实现更正 |
...
> 逐上下文的完整列定义在 §2.1–§2.8，本图只保留**主键、外键与关键判别列**，用于一眼看清 17 张表的全部关联。
