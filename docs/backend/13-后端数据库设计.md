...
| | `resource_allocations` | 独立聚合（配额账本）· ✅ **已建表**（`drizzle/0019`） | §2.1 |
...
**共 18 张表**（15 张属上下文 + 3 张平台表）——这是**设计总量**。**真实建出来的是 16 张**（2026-08-31 按同一条 grep 实测）（`grep -h 'CREATE TABLE' api/drizzle/*.sql` 去掉 `__new_*` 临时表后的全量）：`sandboxes`、`sandbox_state_transitions`、`agent_tasks`、`projects`、`credentials`、`credential_sandbox_bindings`、`runtime_settings`、`runtime_installations`、**`images`、`image_manifests`**（后两张随 2026-08 镜像切片建于 `drizzle/0010`）、**`retained_volumes`**（`drizzle/0017`）、**`automations`、`automation_runs`**（`drizzle/0018`，F21-7）、**`resource_allocations`**（`drizzle/0019`，03 §3 互斥登记切片），以及平台表 `system_settings` 与审计表 `audit_events`。⏳ 仍未建的：`terminal_sessions`、`domain_events`、`nodes`。⚠️ **`audit_events` 不在本文档原来那份 18 张的清单里** —— 它是 §2.8.2 的审计流表，落在 `apps/api` 的平台层；本行按「库里真的有什么」记数，与「设计总量」那一半刻意分开。跨上下文的外键只有 7 条（§2.9 汇总），其余关联一律弱引用——这是 23 §4.2「跨聚合只持 ID」在存储层的落地。
...
>
> **✅ 已落地（`drizzle/0019`）。** schema 在
> `api/packages/modules/sandbox/src/infrastructure/persistence/schema/resource-allocation.sqlite.ts`；
> drizzle 的写法是 `uniqueIndex('uq_alloc_active').on(t.sandboxId).where(...)` —— **`.where()` 那一半是重点**。
> 落地时两处**与本表原设计不符，如实记在这里**：
>
> - ⚠️ **`node_id` 上没有建 FK**，尽管上表画了 `FK→nodes.id`。`nodes` 至今没有建（§2.0 的
>   「⏳ 仍未建」名单里就有它），指向不存在的表的外键在 `foreign_keys=ON` 下建不出来。
>   列、默认值 `'local'` 与 `(node_id, released_at)` 索引都照建；`nodes` 落地时再补 FK。
>   这与 §2.9 对 #7 的记法一致（「目标表根本不存在」）。
> - ⚠️ **`WHERE` 子句丢掉不会有普通测试红** —— 索引会退化成「一个 sandbox 一辈子只能有
>   一条登记」，而只建不销毁的用例一条都察觉不到。
>   `test/integration/resource-allocation-repository.spec.ts` 里专门有一条「释放之后同一个
>   sandbox 可以再登记一次」盯着它（实测：去掉 `where` ⇒ 该条红）。
...
| **`outcome_applied`** | bool | | `false` | `idx(outcome_applied, status)` | **这一列此前只在 03 §8.1 里被要求、本表漏了**（2026-08-31 落地时补）。语义：这条 run 的终态**已经喂给** `Automation.recordOutcome()`（失败计数 / 降频判定）了吗。没有它，「run 已 finalize 但 `recordOutcome` 尚未生效时崩溃」这一路会**漏记一次失败计数**，而仅按 `next_trigger_at` 扫规则永远发现不了它（交叉评审 P2-7）。调度器每轮据此补扫，幂等。⚠️ `skipped`/`missed` 构造时即为 `true` —— 它们本就不改失败计数（I-AUT-1），补扫它们注定是 no-op |
...
**索引**：`(automation_id, triggered_at DESC)`（历史分页）· `(status, retry_at)`（调度器捞待重试项）· `(outcome_applied, status)`（outcome-pending 补扫，03 §8.1）。
...
> #4 / #6 / #7 的**目标或来源表根本不存在**（`terminal_sessions` / `automations` / `nodes` 都还没建，§2.0）；
> ⚠️ **#7 的来源表 `resource_allocations` 已于 `drizzle/0019` 建出，但那条 FK 仍然没有** ——
> 目标表 `nodes` 还是不存在。列与 `(node_id, released_at)` 索引都在，值恒 `'local'`（§2.1.3）。
...
| 7 | `resource_allocations.node_id` | `nodes.id` | **FK** · ⏳ **来源表已建（0019）、FK 未建**（`nodes` 还不存在） | 单机恒 `'local'` |

（上下文**内部**的外键另有 5 条：`sandbox_state_transitions`/**`resource_allocations`（✅ CASCADE，`drizzle/0019` 已生效）**/**`agent_tasks`** → `sandboxes`、`retained_volumes` → `projects`、`image_manifests` → `images`、`credential_sandbox_bindings` → `credentials`、`automation_runs` → `automations`。`agent_tasks` 走 CASCADE：一次运行的作业、日志与产物全都活在那个沙箱里，沙箱没了它们也没了。）
...
| DB 活跃 + 容器查无 | `orphaned` + `released_at=now()` 释放配额 + 发 `SandboxReconciledAsOrphan` 事件（reason='container missing on reconcile'）。⚠️ **本格原写「status→failed」，2026-08-31 按实现回填删去** —— 见下方说明第 3 条 |
...
> **✅ 两条路径都已落地** —— `api/packages/modules/sandbox/src/application/quota-reconciler.ts`：
> **开机全量**（`onApplicationBootstrap`）+ **运行期每 5min 增量**（`setInterval`，`unref()`
> 且 `onModuleDestroy` 清理 —— e2e **全部 spec 跑在同一个 worker 进程里**，不清理会有几十个
> 指向已关闭 DB 的对账器同时醒来）。
>
> ⚠️ **「e2e 单进程」这条保证 2026-09-14 之前其实是空的**：它当时写在
> `vitest.workspace.ts` 的 `poolOptions.forks.singleFork`，而 `createForksPool` 读的是
> **根级** `ctx.config.poolOptions?.forks` —— workspace 模式下 pool 在根级创建一次，
> project 级的 `poolOptions` 根本不参与。实测 e2e 一直是 9 个 worker 并行跑的。
> 没炸只是因为碰 BoxLite 的那几个 e2e 在没有本机镜像的机器上全被 skip 了。
> 现在真正的保证在 `api/scripts/vitest-capped.mjs`（`VITEST_MAX_FORKS=1`），见 25 §1.7。
>
> **增量怎么「只挑长时间未更新的」**（本节原话，也是它不退化成「每 5 分钟一次全量」的关键）：
> 判据是 `now - (上次核对 ?? allocated_at) >= 30min`（`QUOTA_RECONCILE_STALE_MS`），**最旧的
> 先来**，单轮最多 20 条（`QUOTA_RECONCILE_BATCH`）。于是每一轮大约只碰 1/6 的活跃登记，
> 而每条登记仍然半小时被核对一次。「最旧的先来」不是装饰 —— 按库序取会让排在后面的登记
> 在一台繁忙的机器上永远轮不到。
>
> ⚠️ **「上次核对时刻」记在进程内存里，不落库。** §2.1.3 的九列里没有 `last_checked_at`，
> 而为一个纯运行期的调度信息加一列 + 一次迁移，换来的只是「跨重启记得上次核对时刻」——
> 而重启本来就会触发一次**全量**对账，那份记忆到那时正好一文不值。没有记录的登记按
> `allocated_at` 算，于是刚建出来、还在 provision 的沙箱不会被立刻探测。
> ⚠️ **`unknown`（provider 问不出来）不记核对时刻** —— 记了的话，一个 daemon 抖了半分钟的
> 沙箱要再等 30 分钟才有人去看它。
>
> ⚠️ **写登记走 03 §3 的 `SchedulerQueue`（`reconcile` 那一档），不是直接写库。** 不排队的话，
> 「用户按了销毁」与「对账判它是孤儿」可能同时改同一行登记，后到的那次撞上 I-RA-1
> 「释放不可回退」抛异常。登记已经不活跃了就静默 no-op，事件也不发 —— 那意味着销毁抢先
> 跑完了，账本已经是对的。
>
> 落地时三处与本表口径不同，如实记下（**两条路径共用同一段判据，增量这条没有放宽**）：
> - **「容器查无」的判据只认 `instance_missing`**。`instance_exited` / `instance_dead` 仍占着盘
>   （工作区还在、rootfs 还在），判它们孤儿等于宣布停掉的沙箱不占资源 —— 而 `start` 明天还能
>   把它拉回来。
> - **`inspect` 抛异常 ≠ 容器不在**。provider 不可达（docker daemon 没起、boxlite 锁被占）时
>   每一条 inspect 都抛，读成「查无」会在一次抖动里把全部活跃登记判成孤儿、连带放掉一批还
>   活着的沙箱的配额。那一格的处置是**不动** + 一条 warn（与 `RuntimeReconciler`「宁可漏收，
>   不可误删」同一条纪律）。
> - **它不改 `sandboxes.status`** —— 上表那一格原写的「status→failed」**已按实现删去**。把沙箱
>   判死属于 `SandboxHealthMonitor` / 生命周期那条链路；两处都写会在同一个事实上产生两个
>   写者，而这一个还是在启动期、没有任何事件覆盖的那个。
>   ⭐ **但「发事件」与「改状态」是两件事，本轮只做前者**：`SandboxReconciledAsOrphan`
>   **已经有产出方了**（`QuotaReconciler`，与释放登记**同一个事务** —— 所以「账本改了但没人
>   知道」这个中间态不存在）。它由 application 层直接 `publishInTx`，不经聚合：聚合此刻没有
>   任何状态变化可言，为了发一个事件给 `Sandbox` 加一个什么都不改的方法、再靠一次空
>   `saveSync` 把它冲出去，是把事件机制反过来用。
>   消费方按 17 §2 是「审计 / 前端提示」：**审计那一半已接**（`AuditProjector` →
>   `sandbox.reconciled_orphan`，warn 级，actor `system`，03 §7.8 清单已补行）；
>   ⏳ **「前端提示」那一半仍然没有** —— 它需要 `SandboxWsEvent` 加一个成员，那是跨两仓的
>   协议变更（`ws-protocol.ts` + schema hash），本轮刻意不动。今天运维看得到它的地方是
