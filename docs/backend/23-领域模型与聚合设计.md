...
2. **git 凭证** → 跨上下文（project 的 clone 编排、`POST /api/credentials/git/test`）**只经门面** `CREDENTIAL_FACADE.prepareGitAuth(kind, host, scheme)` 拿一个**不透明句柄** `GitAuthContext`；**解密 + 写 `0600` 临时密钥文件 + 组装 env / `GIT_SSH_COMMAND` + `dispose()` 清理全部落在 `credential/infrastructure` 内**，project 侧**只消费句柄、永不持有 `SecretMaterial`**。这消除了"明文不出 credential 边界"与"clone 编排要拼密钥文件"的直接矛盾（详见 27 §5 装配、03 §7.3 落地）。
