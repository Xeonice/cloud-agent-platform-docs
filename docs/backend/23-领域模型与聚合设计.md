...
1. **runtime 凭证** → `materialize()` 把明文注入 sandbox（05 §4）——明文（`SecretMaterial`）不越 `credential/infrastructure` 边界。
2. **git 凭证** → 跨上下文（project 的 clone 编排、`POST /api/credentials/git/test`）**只经门面** `CREDENTIAL_FACADE.prepareGitAuth(kind, host)` 拿一个**不透明句柄** `GitAuthContext`；**解密 + 写 `0600` 临时密钥文件 + 组装 env / `GIT_SSH_COMMAND` + `dispose()` 清理全部落在 `credential/infrastructure` 内**，project 侧**只消费句柄、永不持有 `SecretMaterial`**。这消除了"明文不出 credential 边界"与"clone 编排要拼密钥文件"的直接矛盾（详见 27 §5 装配、03 §7.3 落地）。
...
export interface CredentialFacade {
  prepareGitAuth(kind: 'git-ssh-key' | 'git-https-token', host: string): Promise<GitAuthContext>;
}
```
...
| **I-CRD-2** | **明文永不越过聚合/上下文边界**：明文仅以 `SecretMaterial` 形态存在于 `credential/infrastructure` 内；对外只出两种产物——runtime 的 `materialize()` 注入 sandbox、git 的 `GitAuthContext` 不透明句柄（§8 对外出口）；读模型 `toMaskedView()` 只含掩码 | 类型系统 + 序列化钩子（§8.3）+ boundaries | 05 §4 |
...
**跨上下文门面 `CredentialFacade`（A2 裁决，application 层）**：实现落 `credential/application/credential-facade.adapter.ts`，以 `CREDENTIAL_FACADE` token（`packages/contracts/src/credential-facade.port.ts`，§8 已给定义）装配，与既有 `SANDBOX_FACADE` / `PROJECT_FACADE` 同构。`prepareGitAuth(kind, host)` 内部：① `forKind` 选生效凭证 → ② 校验 `host ∈ allowedHosts`（I-CRD-8）→ ③ 经 `credential/infrastructure` 解密并 materialize 成 `GitAuthContext` 句柄返回。project 的 clone workflow 与 `POST /api/credentials/git/test` **`@Inject(CREDENTIAL_FACADE)` 复用同一路径**（27 §5）。
...
| **D-13** | **`CredentialFacade` 跨上下文门面 + `GitAuthContext` 句柄**（S3） | 消除"明文不出 credential 边界"与"clone 编排要拼密钥文件"的矛盾：materialize（解密+写 `0600`+组 env+`dispose`）落 `credential/infrastructure`，project 只经 `CREDENTIAL_FACADE.prepareGitAuth(kind, host)` 拿不透明句柄；选择服务吃 `kind` 枚举不吃 `RepoUrl` VO，避免 credential/domain import project/domain 撞 boundaries（§8 / §8.5 / A 裁决） |
