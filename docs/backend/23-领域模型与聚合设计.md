...
| 事件 | 何时产生 | 载荷要点 |
|---|---|---|
| `SandboxCreated` | 聚合首次落库（pending） | sandboxId, projectId, **name**（任务显示名，见下） |
...
| `SandboxReconciledAsOrphan` | 对账判定 orphaned | sandboxId / projectId / **name**（同 `SandboxCreated.name` 的理由）/ **status（判定当时的，对账不改它）** / reason。⚠️ **由 application 层（`QuotaReconciler`）直接 `publishInTx`，不经聚合** —— 此刻聚合没有任何状态变化，为发事件给它加一个什么都不改的方法是把事件机制反过来用；与释放登记同一个事务（13 §4） |
| `SandboxDestroyed` | 转入 destroyed 终态 | keepVolume 标记（供 project 上下文登记保留卷） |

⚠️ **`SandboxCreated.name` 为什么在载荷里**：审计流的 `summary` 要求「一行人话，直接上 UI」（13 §2.8.2），而 sandbox 类事件是审计流里数量最多的一类——写 id 那一版让面板整屏都是 UUID。更要紧的是**审计是历史快照**：记的必须是**当时**的名字，任务后来改名（`deriveDefaultTaskName` 只在创建时算一次）甚至沙箱被销毁，那条审计行都该保持原样；回查当前库拿名字会让历史随现状漂移，而那正是审计要防的事。⚠️ 进事件的只有那个**早已在任务列表上公开显示**的名字，`initialPrompt` 原文仍然不出后端（裁决 D-14）。同一条理由适用于 `ProjectCreated.name` 与 `CredentialStored/Revoked` 的 `runtimeId + obtainedVia`（§8.6）。
...
| `ProjectCloneRetried` | ✅ | 审计 `project.clone_retried`。与 `ProjectCreated` 分得开才有意义：同一个基线可能被重试拉过五次，「第几次成的 / 中间隔了多久」正是排查间歇性网络与凭证问题要看的 |
| `ProjectConvertedToEmpty` | ✅ | 前端 invalidate 项目详情；审计 `project.converted_to_empty`（记录曾经的 repoUrl 已丢弃）。⛔ **事件只带远端 host，不带整条 URL**：`RepoUrl` 刻意保留原始串，`https://user:token@host/…` 会把 token 一起带进来，而 05 §4 的脱敏认的是**密钥的形状**，URL userinfo 那一段一条规则都不遮——纪律要在源头成立 |
| `ProjectCloneCancelled` | ✅ | 审计 `project.clone_cancelled`。**不改聚合状态**（落定由 `CloneProjectWorkflow` 的失败路径写成 `failed` + `INTERRUPTED`），所以它是「有人按了停止」的**唯一**记录——没有它，`INTERRUPTED` 与「网线被拔了」事后完全分不开（同 03 §8.3 对 `killed` / `failed` 必须分开的论证）。⚠️ 只有真在 `cloning` 时才发：对已拉完的项目按取消，产品语义是「无事发生」 |
| `ProjectBaselineSynced` | ✅ | 审计 `project.baseline_synced`。它改的是往后每个新 Task 看到的代码，而在跑的 Task 的工作区是创建时的 CoW 快照、不受影响（03 §7.2★）——「两个 Task 坐在不同 commit 上」是刻意不上 UI 的语义，没有这条审计行，那个已知语义就变成一个查不清的怪事 |
| `ProjectArchived` | ✅ | automation 上下文订阅 → 联动禁用规则 |
| `ProjectDeleted` | ✅ | sandbox 上下文订阅 → 级联销毁编排；审计 `project.deleted`。⚠️ **事件在删行之前攒好、与删行同一个事务 publish**，`name` 随事件走——行没了之后没有任何库可以回查（13 §2.8.2「为什么 `subject_id` 不设 FK」） |
...
⚠️ **`RuntimeAuthModeChanged` 的三条落地约定**（此前文档写着它、实现里一条都没有，于是「昨天起全是 401，谁动过 auth-mode」只能从 `runtime_settings.updated_at` 猜出一个时刻，猜不出从哪一档换到哪一档）：

1. **带 `from` / `to` 两档**，`from = null` 即**首次配置**（此前没有 `runtime_settings` 行，没有来处可写）。`null` 与「换到同一档」不是一回事，所以它不能省成一个布尔——审计行要写得出 `account → api-key`，首配那一行则读作「设置为 api-key」。
2. **切到已经生效的那一档不发事件**：那一次 PUT 什么都没改变，落一条 `account → account` 只会让「这一天到底换过几次」更难数。
3. **只有 `PUT /auth-mode` 这条路径发**（`RuntimeSettings.configureFirst`）。credential 上下文首次存凭证时顺手建行的那条路径（`RuntimeSettingsReaderWriter.saveSync`，R-1 ②）跑在**别人的事务**里、从不 `pullEvents()`——让 `create()` 无条件 raise，等于让那条路径静默吞掉一个事件，而被吞掉的事件比压根没有的事件更难查。该路径的可观测性由同一刻的 `CredentialStored` 承担。
...
`CredentialStored`（✅，载荷 `credentialId` + `runtimeId`(git 为 null) + `obtainedVia`）· `CredentialRevoked`（✅，同一份载荷 → sandbox 上下文订阅做联动：对**运行中沙箱**按 `credential_sandbox_bindings` 台账**强制重启/销毁 bound 活沙箱**——env 形态注入进程后外部无法 `unset`，"清除已注入 env"物理做不到；exec 清除失败超时**升级为强制销毁**，不静默记 error，交叉评审 P0-4 / 05 §4）· `CredentialInjected`（✅，记账）· `CredentialExpiring` / `CredentialExpired`（✅ → 投影为 WS `runtime-auth.status_changed`）。

⚠️ **Stored/Revoked 为什么带 `runtimeId + obtainedVia`**：凭证**没有用户起的名字**，审计 `summary`（13 §2.8.2）要认得出对象就只能靠「哪个 runtime + 什么获取方式」这个组合（`保存凭证 claude-code（oauth-device）`）。而 `revoke()` 会把密文擦成 `Erased`（I-CRD-3），回查当前库拿标识会让历史随现状漂移。⛔ **带的是身份，不是内容**：密文、token 片段、`MaskedIdentifier`（SSH 指纹 / token 末四位）一律不上事件——脱敏纪律因此在**源头**成立，而不是靠每个消费者记得删（05 §4）。
...
事件：`ImageRegistered` · `ImageValidated`（携带三级结果）· `ImageActivated` / `ImageDeactivated` · `ImageConfigUpdated` · `ImageDeleted`。

⚠️ **`ImageDeleted` 与 `ImageDeactivated` 必须分得开。** 停用只是把它移出选项列表，历史引用照旧合法（I-IMG-3）；删除是那行 bits 的坐标从平台上消失。事后只看到「这张镜像不见了」而分不清是哪一种，排查方向完全相反。与 `ProjectDeleted` 同一条纪律：事件在 `deleteSync` **之前**攒好、与删行**同一个事务** publish。

⚠️ **每一条都带 `ref`（完整坐标 `registry/repo:tag`），且它是类不是接口。**
· `ref` 的理由：13 §2.8.2 把 `summary` 写成了可验收的一条——**这一列不许出现 UUID**，而镜像的 `manifestId` 正是 UUID。没有这个字段，审计只能写出 `停用镜像 8f3c1d02-…`，而「谁把生产镜像停用了」正是这一档存在的理由。⚠️ 它必须**随事件走**（projector 不回查当前库）：同一个 tag 的现任版本随时会被 `activate` 换掉、整行也可能被删，那条审计行都该保持原样。⚠️ 它是**完整坐标**而不是 `version` 那一段——`ImageRegistered.ref` 此前塞的是 `props.version`，也就是一个孤零零的 `v2`，在面板上和 UUID 一样认不出是哪张。
· 类的理由：平台级 `AuditProjector` 靠 `instanceof` 判别。接口在运行期什么都不是，只能退回按 `type` 字符串比对——而字符串比对在字段改名、事件拆分时**一条编译错误都不会有**，正是审计这一侧最不该有的沉默；其余四个上下文的事件早已都是类。
...
| **I-AUT-10** | **一个已到期的触发槽（`nextTriggerAt <= now`）不许无声消失**：它在被移出调度器扫描面之前，必须在 `automation_runs` 里留下**恰好一行**（triggered / skipped / missed 之一） | 03 §8.1 · 本节下方 |

#### I-AUT-10 的来历与落点（2026-09-04）

**来历是一处真缺陷**（测试 agent 在变异测试里发现并上报，29 §3.3.2b-6 末尾）：
`recordOutcome()` 的 failed/timeout 分支末尾曾无条件重算一次 `nextTriggerAt`
（那行是为「降频/禁用会改变下一次算在哪」写的）。调度器一轮的顺序是
`applyPendingOutcomes() → advanceInFlight() → fireDue()`，于是上一发在前半段落成
`failed` ⇒ 这行把 `nextTriggerAt` 推到 `now` 之后 ⇒ 后半段的 `listDue(now)`
（判据 `enabled AND next_trigger_at <= now`）**再也取不到它** ⇒ 那个到期槽既没触发、
也没有 skipped/missed，**历史里是一个没有任何行的空洞**。同样局面下上一发若是
`success` 会正常触发、若还在跑会留下 `skipped/PREVIOUS_RUNNING` ——
**三条路径给出三种历史**，这是最不该有的那种不一致。

**为什么不写补偿代码**：那行重算本来就不该无条件执行。它想做的两件事都只在**状态真的
翻转**时才需要（转 degraded 要按新频率重算、转 disabled 要置空），普通失败根本不需要动
`nextTriggerAt`。改成与 success 分支同构之后，那个槽留在原地 ⇒ `fireDue()` 照常取到 ⇒
按既有决策表留下 missed / skipped / triggered 之一。**不变量由既有结构自动满足。**

⚠️ **副作用（明确接受）**：一条连续失败的规则在降到 `degraded` 之前**仍按原频率跑**。
降频这道闸由 I-AUT-2 / I-AUT-3 管着且用户看得见（横幅 + webhook），在它之外再叠一层
「失败就顺手退避一格」是重复的，而且不可见。

**落点是结构，不是注释。** `Automation` 里 `_nextTriggerAt` 只有五个写口，每一个都在源码
里带着 `// slot: …` 标记，各自回答「那个槽去哪了」；`automation-slot-writes.spec.ts`
把这张表钉住 —— 新增第六个写口时它会红着要求先回答这个问题：
...
| `slot: recompute` | `recomputeFutureTrigger()` | **结构上只动未到期的槽**（`<= now` 的直接早退） |
| `slot: off-scan` | `disable()` / 连续失败自动禁用 | 整条规则退出扫描面 —— 这是 I-AUT-10 允许的唯一一种「槽消失」，因为它**不是无声的**（`AutomationDisabled` 事件 + `automation.disabled` 审计） |

⇒ 「把一个到期槽往后推」在这个聚合里**没有写口**。`update()`（用户改调度）也走
`recomputeFutureTrigger()`：显式的是「以后怎么跑」，不是「刚才那一发去哪了」，而
`update()` 连一条审计都不写 —— 那一槽照样会被下一轮处理掉并留下一行，之后才按新调度走。

> ⚠️ **与调度器的逐条隔离（03 §8.1「一条坏数据不许让整轮停摆」，H2）不冲突。** 那一处的
> 跳过全发生在 `applyPendingOutcomes` / `advanceInFlight` 两个阶段，而它们**一次都不动
> `nextTriggerAt`** —— 跳过一条 run 不会把任何槽移出扫描面，本不变量的判据是「槽在被移出
> 扫描面之前有没有留下一行」，与「这条 run 这一轮推没推进」是两件事。方向反而是正的：H2
> 修之前，前两个阶段一抛就把 `fireDue()` 整段带走，那一分钟**所有**到期的槽都没人处理。
