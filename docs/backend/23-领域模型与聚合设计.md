...
├─ runtimeId / prompt
├─ schedule: Schedule                 值对象（kind + config + tz + nextOccurrence()）
├─ enabled: boolean
...
| 值对象 | 要点 |
|---|---|
| **`Schedule`** | `{ kind: hourly\|daily\|weekly, config, tz: IANA }` + **`nextOccurrence(after: Date): Date`**。这是本上下文最值得单测的纯函数：夏令时切换按**本地墙钟**语义（"每天 08:00"永远是当地 08:00）、周几多选、月末边界（03 §8.1） |
