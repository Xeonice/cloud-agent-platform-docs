...
- fixture 是**逐字节录制的真实输出**，不允许手写"看起来像"的文本；
...
| T-PRJ-2 | `sourceType='git-clone'` 无 `repoUrl` → 拒绝；`empty` 带 `repoUrl` → 拒绝 | I-PRJ-1 |
| T-PRJ-3 | `empty` 项目的 `cloneStatus` 恒 `ready` 且 `baseline` 为空 | I-PRJ-2 |
| T-PRJ-4 | `cloneStatus='ready'` 且 git-clone 但 `baseline` 为空 → 抛不变量异常 | I-PRJ-3 |
...
| T-CRD-11 | `CredentialSelectionService.forRepo`：ssh URL + 只配了 HTTPS token → 返回 null（而非错选） | 03 §7.3 |
...
| E2E-2-firstConfig | 首次配置后 `runtime_settings` 最终被写入（relay 追平后）；追平前读接口按"唯一凭证"推断，不报错 |
