...
### 1.7 并行度上限（2026-09-14）

`pnpm test` 系列一律经 `scripts/vitest-capped.mjs` 起 vitest，它在子进程启动**之前**设好
`VITEST_MIN_FORKS` / `VITEST_MAX_FORKS`：

| project | 并行度 | 为什么 |
|---|---|---|
| `unit` / `integration` / `contract` | `min(4, 核数/2)` | vitest 默认是 `逻辑核数 − 1`，只看 CPU、不看内存 |
| `e2e` | **1** | e2e 驱动**共享的外部资源**：docker daemon、`:5001` registry，以及 BoxLite —— 每个 `BOXLITE_HOME` 同一时刻只允许 ONE runtime 跨进程存在，分开 fork 会争锁然后 500 |

**为什么要设上限**：每个 fork 是独立 node 进程，各自扛一整套 SWC 转译后的模块图。10 核开发机上
默认设置实测 9 个 worker、峰值 1.6 GB；限到 4 之后峰值 **854 MB**（同一批 1926 条用例，一条不少）。
内存贴顶的机器上那 1.6 GB 足以越线，而 OOM killer 挑的往往**不是测试自己、是旁边的后台进程**
——实测被杀的是正在跑的 dev server，测试本身若无其事地全绿，因果关系因此极难被看见。
⛔ 不按环境分支（本地一套 / CI 一套）：CI runner 的内存只会比开发机更紧。

⚠️⚠️ **这个上限只能从进程外面给，写进 `vitest.workspace.ts` 是死的**（踩了四轮才看明白）：
`createForksPool` 读的是**根级** `ctx.config.poolOptions?.forks`，workspace 模式下 pool 在根级
创建一次，**project 级的 `poolOptions` 与顶层 `maxWorkers` 统统不参与**。写在 project 里不报错、
测试全绿、worker 数一个不少（实测与不设时逐项相同），**没有任何信号告诉你那行配置是死的**。
在 workspace 文件顶部设 `process.env` 同样太晚，`resolveConfig` 早读过了。
只有 `VITEST_MAX_FORKS` 这条环境变量会被 `resolveConfig` 写进 resolved 根配置 —— 正是
`ctx.config` 读的那一份。且 min/max **必须成对给**，只给 max 会直接
`RangeError: options.minThreads and options.maxThreads must not conflict`。

> e2e 那条 `poolOptions.forks.singleFork` 仍留在 `vitest.workspace.ts` 里：语义正确，且将来
> vitest 若修了 project 级 pool 配置它会自动接上 —— 但**当前不产生效果**，文件里已就地标注。
> 因为它一直是死的，`pnpm test`（全量、单进程）没法在一趟里给 e2e 单独设 1，故拆成两趟跑：
> 并行的三个 project 一趟，e2e 单独一趟。CI 本来就是四步分开调的，不受影响。
...
| T-AUT-46 | **一条坏数据不带走整轮**：孤儿补扫 / 在飞推进 / 重试到点三个循环里各摆一条会抛的数据 ⇒ ① 它自己被跳过并留下**带 id** 的一行；② **同一轮里**同循环的下一条、以及后面的阶段照常执行（②才是这条的价值）。另加一格：**取数整批抛**（逐条 try/catch 够不着）时后面的阶段照样跑 | 03 §8.1 |
| T-AUT-47 | **跳过的痕迹既不静默也不刷屏**：同一条连续失败 ⇒ 首轮 `warn`、中间降 `debug`、第 60 轮再 `warn`；中间恢复过一轮、或失败原因变了 ⇒ 立刻重新 `warn`（账本不许把它当成同一次的延续，也不许只增不减） | 03 §8.1 |
