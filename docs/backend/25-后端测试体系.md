...
| 用例 | 断言 |
|---|---|
| T-IMG-15 | `validate` 三级：合法镜像（含 bash + tmux）→ `valid`；**缺 tmux → `invalid` + `errors[]` 含 `IMAGE_TMUX_MISSING`**（2026-08 tmux 升 MUST，04 §7 / testkit IS-05——**原断言是「→ `warning` 且 `supportsTmux=false`、仍可选」，方向已反转，不是笔误**）；有 tmux 但 `supportedRuntimes` 声明的 CLI 未预装 → `valid` + `warnings`（这才是现在的 warning 档）；违反入口约定 → `invalid` + `errors[].path` 非空 |
