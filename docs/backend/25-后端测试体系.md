...
| **T-SBX-31** | **`starting` 五步顺序**（03 §4.3）：断言调用序为 `provider.start` → agent 就绪探测 → `ensureRuntimeInstalled` → `injectCredential` → `bootstrapAgentSession`；**特别断言 `injectCredential` 在 `provider.start` 之后**（此前的设计顺序是错的——`exec` 由 `spawn({tty:false})` 派生，实例没起来就没有 `exec`） | 03 §4.3 · 04 §2.3 |
