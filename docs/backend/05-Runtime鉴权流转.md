...
>
> **② 档的「第二重障碍」已撤销（2026-08 二次纠错）**：本行原写「当前平台 data-plane 客户端的 `exec` 只读 `spec.cmd`，stdin / env / cwd 一律静默丢弃 ⇒ 补齐之前 ② 档不具备可实现性」——**该结论已被推翻**：那是**调错端点**（`/v1/shell/exec`）造成的假象；data-plane 切到 `/v1/bash/exec` 后 `ProcessSpec.stdin` **已可用**（经 `/v1/file/write` + `< file` 重定向合成，有真 fd 0、有真 EOF，且密钥不进命令行），详见 04 §2.3★。⇒ **② 档在通道层面已经跑得通；但它仍然只是可选 / 版本敏感档**——降级的第一重理由（CLI 版本漂移，见上表 ①）原封不动地成立，默认档不变。
>
