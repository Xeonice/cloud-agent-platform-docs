...
>
> **★进度：「不发明进度回调」这条我自己推翻了（2026-09-10 当天）**
>
> 上一版这一格写的是「⛔ **不发明一个** —— 那会逼每个实现去编数字」。**那句对「编造」
> 是对的，对「回调」是错的**：一个**可选的、允许不报**的回调不构成逼迫 —— 测不出来的
> provider 一次都不调即可，调用方退回「已用时长」。而严格版本换来的，是用户面前一条
> **20 分钟的静默进度条**（用户原话：「现在看不到下载了多少的进度」）。
>
> ⇒ `stageImage?(image, onProgress?: (bytesDownloaded: number) => void)`。
>
> | 谁 | 出什么 | 为什么在这一侧 |
> |---|---|---|
> | **provider** | 分子：**本次调用期间新落盘的字节** | 它本来就要读自己的库才能测；⚠️ **基线由它自己减** —— 库里早就有的别的镜像的层，因此不会一上来冒充成进度 |
> | **平台** | 分母：读 registry manifest 求层 size 之和 | provider 不必懂 OCI；⚠️ 必须**先按架构下钻**，否则把 index 里所有平台加起来，进度永远停在 30% |
>
> ⚠️ **报的不是百分比，也不是「这张镜像的进度」** —— provider 不知道总量，一个
> provider 自己编的总量正是这条纪律要防的东西。
>
> ⚠️ **两个数各自允许缺席，缺了就降级、不许编**：分母没有 ⇒ 只报「已下载 X」而
> `progress` 为 `null`；分子一次都不报 ⇒ 退回「期间没有输出，不是卡死」，⛔ 不画 0%。
> 测不出来一律 `null` 而**不是 0** —— 一个 0 会画成卡在 0% 的条，而字节明明在进来
> （同 `imageStaged` 那条「不知道不是 false」）。
>
> ⚠️ **实现侧的代价要记住**：boxlite 那一档的分子来自**读它自己的私有目录布局**
> （SDK 没暴露），升级可能挪走 ⇒ 全部失败路径返回 `null` 让 UI 降级，并有专门的用例
> 把这份耦合钉出来。另：量它必须用 `stat.size` 而**不是 `du`** —— 后者报磁盘块占用，
> 实测高 4.4%（334MB vs 320MB）。
>
