...
| 无头输出（v1.1） | Task 类型=headless && 运行中/完成 | 右侧不显示交互终端，改为只读输出面板：**等宽字体、保留换行不自动折行**（横向滚动超长行）；exit code + 最后 100 行日志（stdout + stderr 合并，UTF-8，按时序倒序）+ [查看完整日志]（打开独立日志查看器，v1.1 调用 `/api/runs/:id/logs`） / [下载] |
