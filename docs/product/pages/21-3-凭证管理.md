...
| 配置 HTTPS Token | **选 host 来源**（GitHub→`github.com` / GitLab→`gitlab.com` / Gitee→`gitee.com` 自动推导 host；或"其他"手填自建 host），**允许添加多个 host**（一条 token 可绑多个仓库主机）→ 附 scope 提示"需 repo 权限" → 粘贴 → [测试连接] → [保存]；展示 Token 尾号 + **host 白名单（明文展示，非敏感）** |
...
flowchart TD
    ...
    I -->|"https:// → HTTPS Token"| J["选来源/host 白名单（可多个）· 粘贴（需 repo 权限）"]
...
- **HTTPS Token 绑 host 白名单（安全边界，技术 03 §7.3）**：一条 Token 只对白名单内的 host 生效——clone/测试连接时若目标 host 不在白名单，**拒绝携带 Token**（不会把 PAT 发给未授权的主机）。host 白名单**明文展示、可编辑**（host 不是密钥）；来源选 GitHub/GitLab/Gitee 自动填 host，自建填"其他"。
