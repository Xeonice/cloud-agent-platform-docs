...
|---|---|---|---|
| **交互式会话**（MVP 默认） | 右侧 xterm 终端里直接与 agent CLI 对话；**可携带可选的初始任务指令，启动时即执行** | idle 空闲回收（默认 30min，可配）；硬超时 24h | sandbox + `spawn({tty:true})` 终端会话 + 可选 `initialPrompt`（文档 06） |
...
```
首次进入              发起 Task（两步向导，继承项目上下文）    运行观察                结束
┌──────────┐   ┌──────────────────────────────┐   ┌─────────────┐   ┌──────────┐
...
设计原则：**鉴权是全局配置一次的事，不是任务创建的流程环节**——向导只有两步（选 Agent → 确认），步骤序列里没有鉴权。仅当所选 runtime 从未配置凭证时，插入一个**无编号的一次性拦截面板**（"首次使用 Codex：完成一次全局登录——此后所有任务自动使用该凭证"），完成后永不再现；用户不需要先理解"什么是凭证管理"，被拦一次顺手配完，全局生效。**项目采用"上下文先行"**——向导继承工作台当前项目作为确认步「项目」行的默认值（可改选、可就地新建，见 §3.2），**不设独立"选项目"步骤**，向导保持两步（默认值哲学：同一项目连发多个 Task 时零重复决策）。
...
**项目选择**：确认步（2/2）为一个**项目下拉行**（选项=全部项目+「＋新建项目…」），默认值=工作台当前项目（上下文继承）；用户可在此改变任务归属项目。下拉底部「＋新建项目…」可直接创建项目，创建后自动回到确认步且新项目已选中，**已填任务指令等表单值不丢**。

**Step 1 · 选择 Agent Runtime** —— 见 §4。

**（无编号）一次性全局登录拦截面板** —— 仅所选 runtime **从未配置凭证**时在两步之间插入，见 §5；配置一次全局生效、永不再现。已有生效凭证时向导就是干净的两步（确认页显示"将以 ✉️ a***@gmail.com 身份运行"，鉴权零感知）。
...
要点：**默认值哲学**——用户真正必须决策的只有 Step 1 的 runtime；**任务指令是唯一鼓励填写但不强制的输入**（可选）；镜像/隔离等级全部有合理默认（AIO 镜像 / aio provider），资源分配由后端根据镜像 resource_defaults 和调度策略自动决定，用户不感知配额；高级选项折叠；鉴权不占步骤（全局一次性），常态发起就是"选 Agent → 确认"两击。

### 3.3 发起后的前后端一体链路

| 步骤 | 用户所见 | 前端落点 | API | 后端流转 |
|---|---|---|---|---|
| 1 | 点击[发起]，向导变阶段进度卡 | `useCreateSandbox` mutation（乐观插入列表项，状态"准备中"） | `POST /api/sandboxes { projectId（确认步项目行所选）, runtime, image, provider, initialPrompt? }` | 校验镜像（04 §7 validate）→ SchedulerQueue → **资源自动分配**（后端根据镜像 resource_defaults 和调度策略，用户无感）→ 返回 202 + sandbox 记录（pending） |
| 2 | 进度条逐阶段更新：✅ 初始化 → 🔄 拉取镜像 → 🔄 **准备工作区** → 🔄 **启动实例**（凭证注入发生在此阶段，用户无感知） | WS 事件驱动 `setQueryData` patch（15 §2.3） | WS `sandbox.status_changed` | provider.create/start（aio/boxlite）+ **项目工作区独立副本挂载专属卷**（pages/21-6 §9）+ `CredentialVault.materialize + injectCredential`（05 §4）；状态机逐步转移并事务双写（03 §4 / 13 §3） |
...
| 4 | 终端展示：**若填了任务指令**则回显内容 + agent 开始执行；**若未填**则显示 agent CLI 欢迎界面等待用户输入 | `TerminalContainer` 渲染 xterm，接收 pty 数据 | data 帧流 | pty 输出 16ms 批量转发（06 §7） |
...
| runtime × 镜像联动 | 确认步的镜像下拉**选项来源 = 镜像管理已启用镜像**（21-4），按所选 runtime 过滤（`supportedRuntimes`，21-4 §9）；**默认平台默认镜像 AIO**；镜像管理中禁用镜像 → 向导下拉自动移除；新注册镜像通过验证 → 下拉出现 |
...
直接进确认步，显示        「首次使用 Codex：完成一次  [重新授权] → 走②流程，
...
```mermaid
flowchart TD
    A["Step1 选中 runtime"] --> B{"查 runtime-auth status<br/>页面首载已预取"}
    B -->|"① 有生效凭证（常态）"| C["直达确认步<br/>显示『将以 a***@gm 身份运行』"]
    B -->|"② 无生效凭证"| D["插入无编号拦截面板<br/>（仅此一次，配置后永不再现）"]
    ...
    H2 --> F
    C --> I["确认步 [发起] → 阶段进度卡<br/>（无等待登录环节，凭证由后端注入）"]
...
**Destroy（销毁）**：弹层二次确认，默认 ☐ 保留工作区成果卷（不勾选才永久删除）；销毁时缓存卷 + 临时卷自动删除（P22 §4.2）。保留的卷在**项目菜单**可见为"已保留卷"（可再次下载/恢复/重新删除，v1.1）。

**Auto-Cleanup（30天自动清理）**：保留的卷>30天自动删除（支持 3/7/30 天可配，P21-7 §3.2 自动化规则的"成果保留期"一致）；删除前可选择导出工作区 diff 或打包（v1.1 成果导出）。

## 7. 其他链路（简版）

| 链路 | 步骤 | 详见 |
|---|---|---|
| 注册自定义镜像 | 设置 → 镜像管理 → 提交 URI → validate 分级反馈（✅/⚠️/❌）→ 出现在发起向导确认步 | pages/21-4、技术 04 §7 |
...
graph TD
    Home["🏠 工作台 /"]
    Init["初始化向导（阻塞·仅首次部署）"]
    Step1["向导 Step1: 选 Runtime"]
    AuthGate["鉴权拦截面板（无编号·仅该 runtime 首次配置时·两种模式均即时完成）"]
    Confirm["向导 Step2: 确认配置"]
    Progress["创建中: 阶段进度卡"]
    ...
    Home -->|"[+ 新任务] / Cmd+K / 空态 CTA（继承当前项目）"| Step1
    Home -->|"顶栏 ⚙️ 设置 / 横幅[授权] / Cmd+K"| Cred
    Home -->|"Cmd+K → Settings"| Img
    Home -->|"Cmd+K / 异常提示[诊断]"| Sys

    Step1 -->|"有生效凭证（常态）：两步直达"| Confirm
    Step1 -->|"该 runtime 从未配置/已过期（仅一次）"| AuthGate
    AuthGate -->|"配置成功(2s 自动)，此后永不再现"| Confirm
...
| 来源 | 目标 | 触发 | 前置条件 | 可返回 |
|---|---|---|---|---|
| 工作台 | 项目创建弹层 | 左侧树底 [＋ 新建项目] / 确认步项目行「＋ 新建项目…」 | — | ✅（关闭；向导来源则回确认步） |
| 工作台 | 项目菜单侧弹层 | 树组头 hover「⋯」 | — | ✅（关闭） |
| 项目创建弹层 | 工作台 | 创建成功（克隆完成）自动选中新项目 | — | — |
| 工作台 | 向导 Step1 | [+ 新任务] / Cmd+K / 空态 CTA | **须已有选中项目**（无项目时先走建项目引导） | ✅（取消） |
...
| Step1 | 确认步 | 选卡片（常态） | 该 runtime 凭证有效——**鉴权零感知，两步直达** | ✅ |
| Step1 | 鉴权拦截面板 | 选卡片（仅一次） | 该 runtime 从未配置或已过期 | ✅ |
| 拦截面板 | 确认步 | 配置成功（此后永不再现） | 后端 status=success | —（自动） |
| 拦截面板 | 凭证管理 | [管理所有凭证] | — | ✅（面板数据保留） |
| 确认步 | 进度卡 | [发起] | 配置完整（默认值即完整） | ❌（创建已触发；取消=终止并自动回收资源，需确认） |
...
| 向导 | **modal 为主**，`/new` 承接深链 | `/new?runtime=codex`（预选 runtime） | modal 刷新即关闭（可接受权衡）；`/new` 路由形式可恢复到 Step1 |
...
- **modal 不堆叠**：向导内需要去凭证管理时**替换为路由页面**（保留浏览器后退），不嵌套 modal；吊销 confirm 是唯一允许的 modal 内二级弹层。
...
| 刷新（向导 modal 中途） | 向导关闭回工作台，流程重来（拦截面板中的配置若后端已完成则不再出现——凭证已入 Vault） |
...
2. **auth helper 编排**（5.2 决策 A）：后端需提供常驻或按需唤起的 auth helper 执行环境，用于跑 CLI 登录流（device-code/setup-token，技术 05 §2）；拦截面板内即时完成鉴权，创建流程不存在等待登录节奏——两种模式都完成于 Step 1→2 之间的一次性面板内，进度卡直接进入"初始化"阶段。
